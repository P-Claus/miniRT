...
/*   Updated: 2024/09/26 18:35:44 by efret            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	del_page_add_spheres(t_mlx_data *data, t_ui_menu_page *page)
{
	...
	while (i < data->scene.nb_of_spheres)
		add_elem_btn(page, "sphere", (t_elem_data){UI_DATA_SPHERE, &data->scene.spheres[i++]}, menu_btn_del_sphere);
}
...
void	del_page_add_cylinders(t_mlx_data *data, t_ui_menu_page *page)
{
	...
	while (i < data->scene.nb_of_cylinders)
		add_elem_btn(page, "cylinder", (t_elem_data){UI_DATA_CYLINDER, &data->scene.cylinders[i++]}, menu_btn_del_cylinder);
}
...
void	del_page_add_planes(t_mlx_data *data, t_ui_menu_page *page)
{
	...
	while (i < data->scene.nb_of_planes)
		add_elem_btn(page, "plane", (t_elem_data){UI_DATA_PLANE, &data->scene.planes[i++]}, menu_btn_del_plane);
}
