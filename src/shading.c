...
/*   Updated: 2024/09/13 00:57:44 by efret            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rgb	phong_shading(t_ray ray, t_hit_info hit, t_scene_info scene)
{
	...
	if (!(vec3_dot(phong.hit_normal, phong.light_dir) < 0 || cast_shadow_ray((t_ray){vec3_sum(hit.coordinates, vec3_scalar(phong.hit_normal, 1e-4)), phong.light_dir}, scene, sqrt(phong.light_dist2))))
	{
