...
/*   Updated: 2024/08/06 13:50:38 by pclaus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_diameter(t_scene_info *scene_info, char *string)
{
	...
	scene_info->sphere.diameter = conversion;
	return (0);
}
...
int	parse_sphere(t_scene_info *scene_info, char *string)
{
	...
	if (count_items_in_split(split, 4) == 1
		|| parse_coordinates(&scene_info->sphere.coordinates, split[1]) == 1
		|| parse_diameter(scene_info, split[2]))
	{
		free_split(split);
		return (1);
	}
	trimmed_rgb = ft_strtrim(split[3], "\n");
	if (parse_rgb(&scene_info->sphere.rgb_code, trimmed_rgb) == 1)
	{
